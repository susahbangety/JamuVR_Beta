...
namespace UnitySimpleLiquid
{
    ...
    public class SplitController : MonoBehaviour
    {
		...
		private void OnDrawGizmosSelected()
        {
            ...
            if (gameObject.name != "Keran")
            {
                Gizmos.color = Color.red;
                GizmosHelper.DrawPlaneGizmos(bottleneckPlane, gizmoTransform);

                // And bottleneck position
                GizmosHelper.DrawSphereOnPlane(bottleneckPlane, BottleneckRadiusWorld, gizmoTransform);
            }
            else
            {
                Gizmos.color = Color.red;
		...
		private RaycastHit FindLiquidContainer(Vector3 splitPos, GameObject ignoreCollision)
		{
			...
			{
				//Ignore ourself
				if (!GameObject.ReferenceEquals(hit.collider.gameObject, ignoreCollision) && (!hit.collider.isTrigger || hit.collider.isTrigger))
				{
					...
					if (liquid && liquid != this)
					{
                        ...
                        if (hit.collider.GetComponent<LiquidContainer>().FillAmountPercent == 0)
                        {
                            hit.collider.GetComponent<LiquidContainer>().LiquidColor = liquidContainer.LiquidColor;
                        }
